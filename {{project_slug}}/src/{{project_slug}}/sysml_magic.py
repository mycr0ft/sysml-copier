...
replaces the prior definition of the same name; other packages are kept.

Line magics (analogues of the OMG Pilot Implementation kernel commands —
see docs/sysml-magics.md for the full official set):

    %sysml_reset                 discard the session model
    %sysml_list [NAME]           list packages, or find elements by exact name
    %sysml_show NAME             print the AST rooted at a named element
                                 (add --json for full JSON representation)
    %sysml_viz NAME [--view V]   PlantUML view of a named element
                                 (views: general, interconnection, action,
                                  package, tree, state, sequence, parametric,
                                  block, internal, relationship, tabular, ...)

Cell options (after `%%sysml`):
    --reset       discard the session model before parsing this cell
    --file PATH   parse SysML from PATH instead of the cell body
    --show        print the round-tripped model text after merge

Parse or semantic errors are reported to stderr without killing the session.
"""
...
class SysMLMagics(Magics):
    ...
    @argument("--file", default=None, metavar="PATH",
              help="parse SysML from PATH instead of the cell body (cell may be '-' placeholder)")
    ...
    def sysml(self, line: str, cell: str):
        ...
        With --file, the cell body may be '-' or blank-only is not allowed by
        IPython; use a single '-' as the body.
        """
        import sys
        ...
        for w in self._analyze(self.model, source):
    ...
    @magic_arguments()
    @argument("--json", action="store_true", help="JSON representation (dump() output piped through json if possible; falls back to text)")
    ...
    @argument("--view", default="general", metavar="VIEW",
              help="general|interconnection|action|package|tree|state|sequence|...")
